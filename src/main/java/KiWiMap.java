...
public class KiWiMap implements CompositionalMap<Integer, Integer> {
    ...
    public void putMany(Map<Integer, Integer> entries) {
        int myVer = kiwi.version.getAndIncrement();
        ...
        for (Map.Entry<Integer, Integer> entry : entries.entrySet()) {
            ...
            while (true) {
                Chunk<Integer, Integer> chunk = kiwi.skiplist.floorEntry(key).getValue();
                chunk = kiwi.iterateChunks(chunk, key);

                if (chunk.creator != null) {
                    chunk = kiwi.rebalance(chunk.creator);
                    if (chunk == null) return;
                    continue;
                }

                int oi = chunk.allocate(key, val);
                if (oi < 0) {
                    chunk = kiwi.rebalance(chunk);
                    if (chunk == null) return;
                    continue;
                }

                chunk.publishPut(new ThreadData.PutData<>(chunk, oi));
                int actualVer = chunk.setVersion(oi, myVer);

                if (actualVer == Chunk.FREEZE_VERSION) {
                    chunk.publishPut(null);
                    chunk = kiwi.rebalance(chunk);
                    if (chunk == null) return;
                    continue;
                }

                chunk.addToList(oi, key);
                chunk.publishPut(null);
                break;
            }
        }
